...
PSSMCalculator::Profile PSSMCalculator::computePSSMFromMSA(size_t setSize, size_t queryLength, const char **msaSeqs, bool wg, float scoreBias, std::vector<double> branchWeights, std::vector<size_t> &indices) {
#ifdef GAP_POS_SCORING
    std::vector<Matcher::result_t> dummy;
    return computePSSMFromMSA(setSize, queryLength, msaSeqs, dummy, wg, scoreBias);
}
